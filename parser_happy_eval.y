...
type LEPTS_AST_D = (Lista_Errores, PilaTabSimb_D)
type PilaTabSimb_D = [TabSimb_D]
type TabSimb_D = [TabSimbElem_D]
type TabSimbElem_D = (String, TabSimbElemEstado)
data TabSimbElemEstado = 
type PosicionMatriz = (Int,Int)
data ValorAsociado = ValorInt Int | ValorChar Char | ValorBool Bool | Vacio
type Matriz_Pos = (PosicionMatriz, ValorAsociado)
data Comportamiento = Default_ | Activar_ | Avanzar_ | Desactivar_

--encontrar_en_alcance :: String -> PilaTabSimb -> Maybe TabSimbElemInfo


--Encuentra el comportamiento del robot que se necesite en la pila

elegir_comportamiento :: LEPTS_AST -> String -> Comportamiento -> Maybe Comp
elegir_comportamiento (errores, pila) nombre comportamiento =
    let comportamientos = encontrar_en_alcance nombre pila
    in encontrar_comportamiento comportamientos comportamiento



--Encuentra el comportamiento del robot que se necesite en la tabla

encontrar_comportamiento :: Maye TabSimbElemInfo -> Comportamiento -> Maybe Comp
encontrar_comportamiento Nothing _ = Nothing
encontrar_comportamiento (Maybe comportamientos) comportamiento =
    case comportamiento of
        Default_ -> case comportamientos of
                        (Palabra comps) -> do lista <- comps
                                              encontrar_codicion lista Default_
                        (Numero comps) -> do lista <- comps
                                             encontrar_codicion lista Default_
                        (Booleanoo comps) -> do lista <- comps
                                                encontrar_codicion lista Default_
        Activar_ -> case comportamientos of
                        (Palabra comps) -> do lista <- comps
                                              encontrar_codicion lista Activar_
                        (Numero comps) -> do lista <- comps
                                             encontrar_codicion lista Activar_
                        (Booleanoo comps) -> do lista <- comps
                                                encontrar_codicion lista Activar_
        Avanzar_ -> case comportamientos of
                        (Palabra comps) -> do lista <- comps
                                              encontrar_codicion lista Avanzar_
                        (Numero comps) -> do lista <- comps
                                             encontrar_codicion lista Avanzar_
                        (Booleanoo comps) -> do lista <- comps
                                                encontrar_codicion lista Avanzar_
        Desactivar_ -> case comportamientos of
                        (Palabra comps) -> do lista <- comps
                                              encontrar_codicion lista Desactivar_
                        (Numero comps) -> do lista <- comps
                                             encontrar_codicion lista Desactivar_
                        (Booleanoo comps) -> do lista <- comps
                                                encontrar_codicion lista Desactivar_
...
--Encuentra en una lista de comportamientos de un robot aquél que se quiere

encontrar_codicion :: [Comp] -> Comportamiento -> Maybe Comp
encontrar_codicion ((Comp condlista listinstrob):lista) cond =
    case cond of
        Activar_ -> case condlista of
                        Activation -> return (Comp condlista listinstrob)
                        _ -> encontrar_codicion lista cond
        Desactivar_ -> case condlista of
                        Deactivation -> return (Comp condlista listinstrob)
                        _ -> encontrar_codicion lista cond
        Avanzar_ -> case condlista of
                        Cond_Expr -> return (Comp condlista listinstrob)
                        _ -> encontrar_codicion lista cond
        Default_ -> case condlista of
                        Default -> return (Comp condlista listinstrob)
                        _ -> encontrar_codicion lista cond
encontrar_codicion [] cond = Nothing


--Actualiza la pila con el comportamiento acutal del robot

insertar_comp :: LEPTS_AST_D -> String -> Comp -> LEPTS_AST_D
insertar_comp (error, (tabla_d:pila_d)) nombre comp = if esta_en_tabla_d tabla_d nombre
                                                      then (reemplazar_comp tabla_d nombre comp):pila_d
                                                      else tabla_d:(insertar_comp (error, pila_d) nombre comp)
insertar_comp [] nombre comp = []



--Reemplaza el comportamiento actual del robot en la tabla

reemplazar_comp :: TabSimb_D -> String -> Comp -> TabSimb_D
reemplazar_comp ((ident, (TabSimbElemEstado posicionMatriz valorAsociado (Maybe comp_d))):tabla) nombre comp = if ident == nombre
                                                    then ((ident, (TabSimbElemEstado posicionMatriz valorAsociado (Maybe comp))):tabla)
                                                    else ((ident, (TabSimbElemEstado posicionMatriz valorAsociado (Maybe comp_d))):(reemplazar_comp tabla nombre comp))
reemplazar_comp [] nombre comp = []


--True si el robot esta en la tabla actual

esta_en_tabla_d :: TabSimb_D -> String -> Bool
esta_en_tabla_d ((ident, algo):tabla) nombre = if ident == nombre
                                               then True
                                               else esta_en_tabla_d tabla nombre
esta_en_tabla_d [] nombre = False




activar :: LEPTS_AST_D -> LEPTS_AST -> String -> LEPTS_AST_D
activar lepts_d lepts nombre = let comportamiento = elegir_comportamiento lepts nombre Activar_
                               in  insertar_comp lepts_d nombre comportamiento

desactivar :: LEPTS_AST_D -> LEPTS_AST -> String -> LEPTS_AST_D
desactivar lepts_d lepts nombre = let comportamiento = elegir_comportamiento lepts nombre Desactivar_
                                  in  insertar_comp lepts_d nombre comportamiento

avanzar :: LEPTS_AST_D -> LEPTS_AST -> String -> LEPTS_AST_D
avanzar lepts_d lepts nombre = let comportamiento = elegir_comportamiento lepts nombre Avanzar_
                               in  insertar_comp lepts_d nombre comportamiento



--Permite encontrar la estructura con la posicion de matriz,
--  valor asociado y comportamiento actual del robot

encontrar_en_alcance_d :: PilaTabSimb_D -> String -> Maybe TabSimbElemEstado
encontrar_en_alcance_d pila_d nombre = msum $ map (lookup nombre) pila_d


--Permite encontrar el valor asociado al robot

encontrar_valor_asociado :: LEPTS_AST_D -> String -> Maybe ValorAsociado
encontrar_valor_asociado (errores, pilatabsimb_d) nombre = encontrar_en_alcance_d pilatabsimb_d nombre


--Permite evaluar las expresiones con booleanos

eval_bool :: LEPTS_AST_D -> Expr -> Maybe Bool
eval_bool lepts_d (Booleano b) = return b
eval_bool lepts_d (And_ b1 b2) = do p1 <- eval_bool lepts b1
                            p2 <- eval_bool lepts b2
                            return (p1 && p2)
eval_bool lepts_d (Or_ b1 b2) = do p1 <- eval_bool lepts b1
                           p2 <- eval_bool lepts b2
                           return (p1 || p2)
eval_bool lepts_d (Nega b1) = do p1 <- eval_bool lepts b1
                         return $ not p1
eval_bool lepts_d (Parentesis expr) = do exp2 <- eval_bool lepts expr
                                 return exp2
eval_bool lepts_d (Mayor expr1 expr2) = do exp1 <- eval_int lepts expr1
                                   exp2 <- eval_int lepts expr2
                                   return (exp1 > exp2)
eval_bool lepts_d (MayorEqu expr1 expr2) = do exp1 <- eval_int lepts expr1
                                      exp2 <- eval_int lepts expr2
                                      return (exp1 >= exp2)
eval_bool lepts_d (Menor expr1 expr2) = do exp1 <- eval_int lepts expr1
                                   exp2 <- eval_int lepts expr2
                                   return (exp1 < exp2)
eval_bool lepts_d (MenorEqu expr1 expr2) = do exp1 <- eval_int lepts expr1
                                      exp2 <- eval_int lepts expr2
                                      return (exp1 <= exp2)
eval_bool lepts_d (Variabl (Var_C nombre)) = do valorT <- encontrar_valor_asociado lepts_d nombre
                                               case valorT of
                                                    ValorBool valor -> return valor
                                                    _ -> Nothing
eval_bool lepts_d (Expr_Me_ me) = do valorT <- encontrar_valor_asociado lepts_d "me"
                                               case valorT of
                                                    ValorBool valor -> return valor
                                                    _ -> Nothing
eval_bool lepts_d _ = Nothing


--Permite evaluar las expresiones con enteros

eval_int :: LEPTS_AST_D -> Expr -> Maybe Int
eval_int lepts_d (Numer algo) = return algo
eval_int lepts_d (Nega expr) = do exp <- eval_int lepts expr
                          return (0 - exp)
eval_int lepts_d (Modu expr1 expr2) = do exp1 <- eval_int lepts expr1
                                 exp2 <- eval_int lepts expr2
                                 if (expr2 == 0) 
                                 then Nothing
                                 else return $ mod exp1 exp2
eval_int lepts_d (Produ expr1 expr2) = do exp1 <- eval_int lepts expr1
                                  exp2 <- eval_int lepts expr2
                                  return $ exp1 * exp2
eval_int lepts_d (Divi expr1 expr2) = do exp1 <- eval_int lepts expr1
                                 exp2 <- eval_int lepts expr2
                                 if (expr2 == 0) 
                                 then Nothing
                                 else return $ div exp1 exp2
eval_int lepts_d (Parentesis expr) = do exp2 <- eval_int lepts expr
                                return exp2
eval_int lepts_d (Variabl (Var_C nombre)) = do valorT <- encontrar_valor_asociado lepts_d nombre
                                               case valorT of
                                                    ValorInt valor -> return valor
                                                    _ -> Nothing
eval_int lepts_d (Expr_Me_ me) = do valorT <- encontrar_valor_asociado lepts_d "me"
                                               case valorT of
                                                    ValorInt valor -> return valor
                                                    _ -> Nothing
eval_int lepts_d _ = Nothing
